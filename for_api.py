...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('app.log'),  # Сохранять логи в файл
        logging.StreamHandler()  # Выводить логи в консоль
    ]
)
...
def filter_dot(result_old=False):
    if result_old is None or not isinstance(result_old, dict):
        print("result is not defined or is of wrong type")
        return None
...
def all_text_and_sentence_and_word(result=False):
    ...
    if result is None:  # Проверяйте на None
        return None
...
def stt(audio_file, model):
    ...
    if result is None or 'segments' not in result:
        print("Transcription failed or segments not found.")
        return None
...
def get_definition(string):
    ...
    try:
        ...
        # Цикл до тех пор, пока не будет найдено определение
        for _ in range(3):  # Попробуем максимум 3 раза
            try:
                p = wikipedia.summary(wikipedia.search(s)[0])
                break  # Если успешно, выходим из цикла
            except (wikipedia.DisambiguationError, IndexError):
                s = random.choice(list_of_refers)
        else:  # Если вышли из цикла, не найдя определение
...
def audio_to_text(path):
    try:
        ...
        if 'all_text' not in res:
            logging.error(f"Key 'all_text' not found in result: {res}")
            return {}  # Возвращаем пустой словарь, если ключ не найден

        parts, tet, qwq = get_parts_of_text(res['all_text'])
        text = ''.join(res['all_text'])
        ...
        logging.error(f"Error processing audio file {path}: {str(e)}")
